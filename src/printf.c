...
*/
void sqlite4AppendSpace(StrAccum *pAccum, int N){
    ...
    switch( xtype ){
      ...
      case etRADIX:
        ...
        if( precision<etBUFSIZE-10 ){
          ...
          nOut = precision + 10;
          zOut = zExtra = sqlite4Malloc(0, nOut);
          if( zOut==0 ){
      ...
      case etGENERIC:
        ...
        */
        ...
        if( e2+precision+width > etBUFSIZE - 15 ){
          bufpt = zExtra = sqlite4Malloc(0, e2+precision+width+15 );
          if( bufpt==0 ){
            pAccum->mallocFailed = 1;
            return;
          }
      ...
      case etSQLESCAPE3: {
        ...
        n += i + 1 + needQuote*2;
        if( n>etBUFSIZE ){
          bufpt = zExtra = sqlite4Malloc(0, n);
          if( bufpt==0 ){
            pAccum->mallocFailed = 1;
            return;
          }
  ...
  if( p->nChar+N >= p->nAlloc ){
    ...
    if( !p->useMalloc ){
      ...
      if( p->useMalloc==1 ){
        ...
        zNew = sqlite4_realloc(0, zOld, p->nAlloc);
      }
      if( zNew ){
  ...
  if( p->zText ){
    ...
    if( p->useMalloc && p->zText==p->zBase ){
      if( p->useMalloc==1 ){
        ...
        p->zText = sqlite4_malloc(0, p->nChar+1);
      }
      if( p->zText ){
...
** %-conversion extensions.
*/
char *sqlite4MPrintf(sqlite4 *db, const char *zFormat, ...){
  va_list ap;
  char *z;
  va_start(ap, zFormat);
...
**
*/
char *sqlite4MAppendf(sqlite4 *db, char *zStr, const char *zFormat, ...){
  va_list ap;
  char *z;
  va_start(ap, zFormat);
  z = sqlite4VMPrintf(db, zFormat, ap);
...
*/
void sqlite4DebugPrintf(const char *zFormat, ...){
  va_list ap;
  StrAccum acc;
  char zBuf[500];
