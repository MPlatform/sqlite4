...
*/
static sqlite4_mutex *os2MutexAlloc(int iType){
  sqlite4_mutex *p = NULL;
  switch( iType ){
    ...
    default: {
      ...
      static sqlite4_mutex staticMutexes[6] = {
      ...
      if ( !isInit ){
        ...
        sqlite4_snprintf( sizeof(name), name, "\\SEM32\\SQLITE%04x",
                          ppib->pib_ulpid );
        while( !isInit ){
          mutex = 0;
...
*/
static void os2MutexFree(sqlite4_mutex *p){
#ifdef SQLITE_DEBUG
  TID tid;
  PID pid;
  ULONG ulCount;
...
*/
static int os2MutexHeld(sqlite4_mutex *p){
  TID tid;
  PID pid;
  ULONG ulCount;
  PTIB ptib;
...
static int os2MutexNotheld(sqlite4_mutex *p){
  TID tid;
  PID pid;
  ULONG ulCount;
  PTIB ptib;
...
static void os2MutexTrace(sqlite4_mutex *p, char *pAction){
  TID   tid;
  PID   pid;
  ULONG ulCount;
  DosQueryMutexSem(p->mutex, &pid, &tid, &ulCount);
...
*/
static void os2MutexEnter(sqlite4_mutex *p){
...
static int os2MutexTry(sqlite4_mutex *p){
  int rc = SQLITE_BUSY;
  assert( p->id==SQLITE_MUTEX_RECURSIVE || os2MutexNotheld(p) );
...
*/
static void os2MutexLeave(sqlite4_mutex *p){
