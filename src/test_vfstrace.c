...
** sqlite4_open_v2() or by making the trace VFS be the default (by setting
...
** and the SQLite amalgamation source files (sqlite4.c, sqlite4.h) into
** the working directory.  Then compile using a command like the following:
**
**    gcc -o sqlite4 -Os -I. -DSQLITE_ENABLE_VFSTRACE \
**        -DSQLITE_THREADSAFE=0 -DSQLITE_ENABLE_FTS3 -DSQLITE_ENABLE_RTREE \
**        -DHAVE_READLINE -DHAVE_USLEEP=1 \
**        shell.c test_vfstrace.c sqlite4.c -ldl -lreadline -lncurses
**
...
**         shell.c test_vfstrace.c sqlite4.c -ldl -lpthread
**
...
struct vfstrace_info {
  sqlite4_vfs *pRootVfs;              /* The underlying real VFS */
  int (*xOut)(const char*, void*);    /* Send output here */
  void *pOutArg;                      /* First argument to xOut */
  const char *zVfsName;               /* Name of this trace-VFS */
  sqlite4_vfs *pTraceVfs;             /* Pointer back to the trace VFS */
};

/*
** The sqlite4_file object for the trace VFS
*/
...
struct vfstrace_file {
  ...
  sqlite4_file *pReal;      /* The real underlying file */
};

/*
** Method declarations for vfstrace_file.
*/
static int vfstraceClose(sqlite4_file*);
static int vfstraceRead(sqlite4_file*, void*, int iAmt, sqlite4_int64 iOfst);
static int vfstraceWrite(sqlite4_file*,const void*,int iAmt, sqlite4_int64);
static int vfstraceTruncate(sqlite4_file*, sqlite4_int64 size);
static int vfstraceSync(sqlite4_file*, int flags);
static int vfstraceFileSize(sqlite4_file*, sqlite4_int64 *pSize);
static int vfstraceLock(sqlite4_file*, int);
static int vfstraceUnlock(sqlite4_file*, int);
static int vfstraceCheckReservedLock(sqlite4_file*, int *);
static int vfstraceFileControl(sqlite4_file*, int op, void *pArg);
static int vfstraceSectorSize(sqlite4_file*);
static int vfstraceDeviceCharacteristics(sqlite4_file*);
static int vfstraceShmLock(sqlite4_file*,int,int,int);
static int vfstraceShmMap(sqlite4_file*,int,int,int, void volatile **);
static void vfstraceShmBarrier(sqlite4_file*);
static int vfstraceShmUnmap(sqlite4_file*,int);

/*
** Method declarations for vfstrace_vfs.
*/
static int vfstraceOpen(sqlite4_vfs*, const char *, sqlite4_file*, int , int *);
static int vfstraceDelete(sqlite4_vfs*, const char *zName, int syncDir);
static int vfstraceAccess(sqlite4_vfs*, const char *zName, int flags, int *);
static int vfstraceFullPathname(sqlite4_vfs*, const char *zName, int, char *);
static void *vfstraceDlOpen(sqlite4_vfs*, const char *zFilename);
static void vfstraceDlError(sqlite4_vfs*, int nByte, char *zErrMsg);
static void (*vfstraceDlSym(sqlite4_vfs*,void*, const char *zSymbol))(void);
static void vfstraceDlClose(sqlite4_vfs*, void*);
static int vfstraceRandomness(sqlite4_vfs*, int nByte, char *zOut);
static int vfstraceSleep(sqlite4_vfs*, int microseconds);
static int vfstraceCurrentTime(sqlite4_vfs*, double*);
static int vfstraceGetLastError(sqlite4_vfs*, int, char*);
static int vfstraceCurrentTimeInt64(sqlite4_vfs*, sqlite4_int64*);
static int vfstraceSetSystemCall(sqlite4_vfs*,const char*, sqlite4_syscall_ptr);
static sqlite4_syscall_ptr vfstraceGetSystemCall(sqlite4_vfs*, const char *);
static const char *vfstraceNextSystemCall(sqlite4_vfs*, const char *zName);
...
static void vfstrace_printf(
  ...
  char *zMsg;
  va_start(ap, zFormat);
  zMsg = sqlite4_vmprintf(zFormat, ap);
  va_end(ap);
...
static void vfstrace_print_errcode(
  ...
  switch( rc ){
    ...
    default: {
       sqlite4_snprintf(sizeof(zBuf), zBuf, "%d", rc);
       zVal = zBuf;
       break;
    }
  }
...
*/
static int vfstraceClose(sqlite4_file *pFile){
  ...
  if( rc==SQLITE_OK ){
    sqlite4_free((void*)p->base.pMethods);
    p->base.pMethods = 0;
  }
  return rc;
}
...
*/
static int vfstraceTruncate(sqlite4_file *pFile, sqlite_int64 size){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceSync(sqlite4_file *pFile, int flags){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
  int i;
  ...
  if( flags & ~(SQLITE_SYNC_FULL|SQLITE_SYNC_DATAONLY) ){
    sqlite4_snprintf(sizeof(zBuf)-i, &zBuf[i], "|0x%x", flags);
  }
...
*/
static int vfstraceFileSize(sqlite4_file *pFile, sqlite_int64 *pSize){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceLock(sqlite4_file *pFile, int eLock){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceUnlock(sqlite4_file *pFile, int eLock){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceCheckReservedLock(sqlite4_file *pFile, int *pResOut){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceFileControl(sqlite4_file *pFile, int op, void *pArg){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
  char zBuf[100];
  ...
  switch( op ){
    ...
    case SQLITE_FCNTL_SIZE_HINT: {
      sqlite4_snprintf(sizeof(zBuf), zBuf, "SIZE_HINT,%lld",
                       *(sqlite4_int64*)pArg);
      zOp = zBuf;
      break;
    }
    case SQLITE_FCNTL_CHUNK_SIZE: {
      sqlite4_snprintf(sizeof(zBuf), zBuf, "CHUNK_SIZE,%d", *(int*)pArg);
      zOp = zBuf;
      break;
    }
    ...
    default: {
      sqlite4_snprintf(sizeof zBuf, zBuf, "%d", op);
      zOp = zBuf;
      break;
    }
  }
  ...
  if( op==SQLITE_FCNTL_VFSNAME && rc==SQLITE_OK ){
    *(char**)pArg = sqlite4_mprintf("vfstrace.%s/%z",
...
*/
static int vfstraceSectorSize(sqlite4_file *pFile){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceDeviceCharacteristics(sqlite4_file *pFile){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceShmLock(sqlite4_file *pFile, int ofst, int n, int flags){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
  char zLck[100];
  ...
  if( flags & ~(0xf) ){
     sqlite4_snprintf(sizeof(zLck)-i, &zLck[i], "|0x%x", flags);
  }
...
static void vfstraceShmBarrier(sqlite4_file *pFile){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  vfstrace_printf(pInfo, "%s.xShmBarrier(%s)\n", pInfo->zVfsName, p->zFName);
  p->pReal->pMethods->xShmBarrier(p->pReal);
}
static int vfstraceShmUnmap(sqlite4_file *pFile, int delFlag){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
static int vfstraceOpen(
  ...
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
  p->pInfo = pInfo;
  ...
  if( p->pReal->pMethods ){
    sqlite4_io_methods *pNew = sqlite4_malloc( sizeof(*pNew) );
    const sqlite4_io_methods *pSub = p->pReal->pMethods;
...
** returning.
*/
static int vfstraceDelete(sqlite4_vfs *pVfs, const char *zPath, int dirSync){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
  int rc;
...
*/
static void *vfstraceDlOpen(sqlite4_vfs *pVfs, const char *zPath){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
*/
static void vfstraceDlError(sqlite4_vfs *pVfs, int nByte, char *zErrMsg){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
*/
static void (*vfstraceDlSym(sqlite4_vfs *pVfs,void *p,const char *zSym))(void){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
*/
static void vfstraceDlClose(sqlite4_vfs *pVfs, void *pHandle){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
** random data.
*/
static int vfstraceRandomness(sqlite4_vfs *pVfs, int nByte, char *zBufOut){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
** actually slept.
*/
static int vfstraceSleep(sqlite4_vfs *pVfs, int nMicro){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
*/
static int vfstraceCurrentTime(sqlite4_vfs *pVfs, double *pTimeOut){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
  return pRoot->xCurrentTime(pRoot, pTimeOut);
}
static int vfstraceCurrentTimeInt64(sqlite4_vfs *pVfs, sqlite4_int64 *pTimeOut){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
*/
static int vfstraceGetLastError(sqlite4_vfs *pVfs, int iErr, char *zErr){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
static int vfstraceSetSystemCall(
  sqlite4_vfs *pVfs,
  const char *zName,
  sqlite4_syscall_ptr pFunc
){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
  return pRoot->xSetSystemCall(pRoot, zName, pFunc);
}
static sqlite4_syscall_ptr vfstraceGetSystemCall(
  sqlite4_vfs *pVfs,
  const char *zName
){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
...
static const char *vfstraceNextSystemCall(sqlite4_vfs *pVfs, const char *zName){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  sqlite4_vfs *pRoot = pInfo->pRootVfs;
...
int vfstrace_register(
  ...
  pRoot = sqlite4_vfs_find(zOldVfsName);
  if( pRoot==0 ) return SQLITE_NOTFOUND;
  nName = strlen(zTraceName);
  nByte = sizeof(*pNew) + sizeof(*pInfo) + nName + 1;
  pNew = sqlite4_malloc( nByte );
  ...
  return sqlite4_vfs_register(pNew, makeDefault);
}
