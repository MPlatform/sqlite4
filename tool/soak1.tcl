...
proc generate_makefile {pattern} {
  ...
  foreach line [split $maketxt \n] {
    if {[regexp {^ *#? *OPTS[ =+]} $line]} {
      if {!$seen_opt} {
         puts $out "OPTS += -DSQLITE4_NO_SYNC=1"
         foreach x $pattern {
           puts $out "OPTS += -D$x"
         }
         set seen_opt 1
...
scenario 2 {Everything} {
  SQLITE4_DEBUG=1
  SQLITE4_MEMDEBUG=1
  SQLITE4_ENABLE_MEMORY_MANAGEMENT=1
  SQLITE4_ENABLE_COLUMN_METADATA=1
  SQLITE4_ENABLE_LOAD_EXTENSION=1 HAVE_DLOPEN=1
  SQLITE4_ENABLE_MEMORY_MANAGEMENT=1
}
scenario 3 {Customer-1} {
  SQLITE4_DEBUG=1 SQLITE4_MEMDEBUG=1
  SQLITE4_THREADSAFE=1 SQLITE4_OS_UNIX=1
  SQLITE4_DISABLE_LFS=1
  SQLITE4_DEFAULT_AUTOVACUUM=1
  SQLITE4_DEFAULT_PAGE_SIZE=1024
  SQLITE4_MAX_PAGE_SIZE=4096
  SQLITE4_DEFAULT_CACHE_SIZE=64
  SQLITE4_DEFAULT_TEMP_CACHE_SIZE=32
  SQLITE4_TEMP_STORE=3
  SQLITE4_OMIT_PROGRESS_CALLBACK=1
  SQLITE4_OMIT_LOAD_EXTENSION=1
  SQLITE4_OMIT_VIRTUALTABLE=1
  SQLITE4_ENABLE_IOTRACE=1
}
scenario 4 {Small-Cache} {
  ...
  SQLITE4_DEFAULT_AUTOVACUUM=1
  SQLITE4_DEFAULT_PAGE_SIZE=1024
  SQLITE4_MAX_PAGE_SIZE=2048
  SQLITE4_DEFAULT_CACHE_SIZE=13
  SQLITE4_DEFAULT_TEMP_CACHE_SIZE=11
  SQLITE4_TEMP_STORE=1
}
