...
** to the sqlite4Fts2InitHashTable() function (e.g. 'fts2_tokenizer').
**
...
static void scalarFunc(
  ...
  pHash = (fts2Hash *)sqlite4_user_data(context);
  ...
  if( argc==2 ){
    ...
    pOld = sqlite4Fts2HashInsert(pHash, (void *)zName, nName, pPtr);
    if( pOld==pPtr ){
      sqlite4_result_error(context, "out of memory", -1);
      return;
    }
  }else{
    pPtr = sqlite4Fts2HashFind(pHash, zName, nName);
    if( !pPtr ){
...
static void testFunc(
  ...
  pHash = (fts2Hash *)sqlite4_user_data(context);
  p = (sqlite4_tokenizer_module *)sqlite4Fts2HashFind(pHash, zName, nName+1);
...
void sqlite4Fts2SimpleTokenizerModule(sqlite4_tokenizer_module const**ppModule);
...
**
**    sqlite4Fts2HashInit(pHash, FTS2_HASH_STRING, 1);
**
